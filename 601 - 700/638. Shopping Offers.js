 ...
 * @return {number}
 */
const dfs = (price, offers, target, currentPrice, cb) => {
    var availableOffers = offers.filter((offer) => {
    ...
    for (let offer of availableOffers) {
        ...
        dfs(price, availableOffers, afterPick, currentPrice + offer[offer.length - 1], cb);
    }
}
...
var shoppingOffers = (price, special, needs) => {
    ...
    const addToRes = price => {
        res = Math.min(res, price);
    }
    dfs(price, special, needs, 0, addToRes);
    return res;
};