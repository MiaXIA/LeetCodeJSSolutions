...
var findRotateSteps = (ring, key) => {
    ...
    for (let r = 1; r < height; r += 1) {
        ...
        for (let col = 0; col < width; col += 1) {
            ...
            let minArr = [];
            for (let prevColIndex = 0; prevColIndex < width; prevColIndex += 1) {
                let val = dp[r - 1][prevColIndex];
                if (val === Infinity) continue;
                minArr.push(val + Math.min(Math.abs(prevColIndex - col), Math.abs(width - Math.abs(prevColIndex - col))));
            }
